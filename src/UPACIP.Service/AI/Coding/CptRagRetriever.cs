...
/// <summary>
/// Retrieves CPT coding guideline chunks from the pgvector <c>coding_guideline_embeddings</c>
/// index to ground AI procedure code mapping (US_048, AIR-R01–AIR-R05).
///
/// Workflow:
/// <list type="number">
///   <item>Generate a 384-dim embedding of the concatenated procedure texts using
...
/// </summary>
public sealed class CptRagRetriever
{
    // ─────────────────────────────────────────────────────────────────────────
    // Constants (mirrored in cpt-coding-guardrails.json)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private readonly ILogger<CptRagRetriever>  _logger;

    // ─────────────────────────────────────────────────────────────────────────
    // Constructor
    // ─────────────────────────────────────────────────────────────────────────

    public CptRagRetriever(
        IHttpClientFactory       httpClientFactory,
        IVectorSearchService     vectorSearch,
        ILogger<CptRagRetriever> logger)
    {
    ...
    /// <summary>
    /// Retrieves CPT coding guideline chunks relevant to the given procedure texts.
    ///
    /// Returns an empty string on any failure to preserve pipeline availability.
    /// </summary>
    /// <param name="procedureTexts">
    /// Collection of procedure description strings to use as the retrieval query.
    ...
    public async Task<string> RetrieveContextAsync(
        IEnumerable<string> procedureTexts,
        Guid                correlationId,
        CancellationToken   ct = default)
    {
        try
        {
            var queryText = string.Join("; ", procedureTexts
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(ex,
                "CptRagRetriever: RAG retrieval failed; continuing without context. " +
        ...
        CancellationToken ct)
    {
        ...
        {
            _logger.LogDebug(ex,
                "CptRagRetriever: embedding generation failed. CorrelationId={CorrelationId}",
                correlationId);
            return null;
        }
    }
