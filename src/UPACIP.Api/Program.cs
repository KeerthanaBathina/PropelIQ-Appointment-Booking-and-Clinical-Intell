...
// Background services that throw unhandled exceptions should log but NOT bring down the host.
// Redis-unavailable scenarios (EC-1 resilience) are the primary driver of this setting.
...
// Multi-tenancy — Phase 1: single static tenant (NFR-027).
// Register ITenantProvider before ApplicationDbContext so TenantContext can be injected.
...
// Performance and monitoring singletons required by multiple services.
builder.Services.Configure<UPACIP.Service.Performance.Models.PerformanceOptions>(
    builder.Configuration.GetSection(UPACIP.Service.Performance.Models.PerformanceOptions.SectionName));
builder.Services.Configure<UPACIP.Service.Monitoring.Models.DegradationOptions>(
    builder.Configuration.GetSection(UPACIP.Service.Monitoring.Models.DegradationOptions.SectionName));
builder.Services.Configure<UPACIP.Service.Monitoring.Models.MonitoringOptions>(
    builder.Configuration.GetSection(UPACIP.Service.Monitoring.Models.MonitoringOptions.SectionName));
...
// ── Supplemental registrations (services consumed transitively but not wired above) ─────────
...
// PatientProfileCacheService is a concrete dependency of CacheInvalidationCoordinator
builder.Services.AddScoped<UPACIP.Service.Caching.PatientProfileCacheService>();
builder.Services.AddSingleton<UPACIP.Service.AiSafety.IAiRateLimiter,
...
builder.Services.AddScoped<UPACIP.Service.AiAudit.IAiAuditService,
    UPACIP.Service.AiAudit.AiAuditService>();
...
builder.Services.AddScoped<UPACIP.Service.AiSafety.IContentFilterService,
...
builder.Services.AddScoped<UPACIP.Service.Resilience.IExternalServiceResilienceProvider,
    UPACIP.Service.Resilience.ExternalServiceResilienceProvider>();
builder.Services.AddSingleton<UPACIP.Service.FeatureFlags.IFeatureFlagService,
    UPACIP.Service.FeatureFlags.FeatureFlagService>();
...
builder.Services.AddSingleton<UPACIP.Service.Security.IInputSanitizer,
    UPACIP.Service.Security.InputSanitizer>();
builder.Services.AddScoped<UPACIP.Service.Monitoring.IOutageAlertService,
    UPACIP.Service.Monitoring.OutageAlertService>();
// PatientDataDeletionService concrete dependencies
...
// PatientDataExportService concrete dependencies
...
// ── Additional service registrations discovered via DI validation ──────────────────────────
...
builder.Services.AddScoped<UPACIP.Service.Backup.IBackupExecutor,
    UPACIP.Service.Backup.BackupExecutor>();
builder.Services.AddSingleton<UPACIP.Service.Import.ICsvParser,
...
builder.Services.AddScoped<UPACIP.Service.Resilience.ITransientFaultClassifier,
    UPACIP.Service.Resilience.TransientFaultClassifier>();
builder.Services.AddScoped<UPACIP.Service.Backup.IBackupEncryptionService,
    UPACIP.Service.Backup.BackupEncryptionService>();
builder.Services.AddScoped<UPACIP.Service.Backup.IBackupReplicationService,
    UPACIP.Service.Backup.BackupReplicationService>();
builder.Services.AddScoped<UPACIP.Service.Backup.IBackupRetentionService,
...
builder.Services.AddScoped<UPACIP.Service.Auth.IAccountLockoutAuditHandler,
    UPACIP.Service.Auth.AccountLockoutAuditHandler>();

// Backup/recovery infrastructure — concrete classes injected by other services
builder.Services.Configure<UPACIP.Service.Backup.Models.WalArchivalOptions>(
    builder.Configuration.GetSection(UPACIP.Service.Backup.Models.WalArchivalOptions.SectionName));
builder.Services.Configure<UPACIP.Service.Backup.Models.BackupOptions>(
    builder.Configuration.GetSection(UPACIP.Service.Backup.Models.BackupOptions.SectionName));
